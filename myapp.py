...
if (file!= None):
    ...
    with tab2:
        ...
        toprows = st.slider('Number of rows your want',1,data.shape[0], key ='topslider')
        st.dataframe(data.head(toprows))
        st.subheader(':grey[Bottom Rows]')
        bottomrows = st.slider('Number of rows your want',1,data.shape[1], key='bottomslider')
    ...
    with st.expander('Value Count'):
        ...
        if(count==True):
            ...
            st.dataframe(result)
            st.subheader('Visualizatio', divider='grey')
    ...
    with st.expander('Group By your columns'):
        ...
        if (groupby_cols):
            ...
            if(graphs=='line'):
                ...
                path =st.multiselect('Choose you path', options=list(result.columns))
